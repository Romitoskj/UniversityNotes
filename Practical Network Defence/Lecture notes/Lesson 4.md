...
**2. IPv6 Multicast Addresses** IPv6 entirely eliminates the "broadcast" address, replacing it with Multicast (one-to-many communication).
...
**3. IPv6 Header vs. IPv4 Header** The IPv6 header was heavily simplified to improve processing speed and take advantage of 64-bit CPUs. The IPv6 header has a **fixed size of 40 bytes**.
...
**4. Fragmentation in IPv6** A major difference from IPv4 is that **intermediate IPv6 routers do not fragment packets**. Router fragmentation was removed because it slows down packet processing and risks Denial of Service (DoS) attacks.
...
**5. IPv6 Extension Headers (EHs)** Instead of cramming optional data into the main IP header, IPv6 introduces a highly flexible modular system called Extension Headers.
