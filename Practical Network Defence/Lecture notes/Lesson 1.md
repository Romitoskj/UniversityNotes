...
- **Switches & Broadcast Domains:** An Ethernet network forms a broadcast domain. **Switches** help segment this network by learning source MAC addresses and storing them in **CAM tables**. Switches only replicate traffic to the specific segment where the destination MAC resides, limiting unnecessary packet explosion (broadcast messages are an exception and get replicated to all). Hosts uses ARP tables to associate other host IPs with their MAC address.
...
- **IP Addresses:** Provide **logical addressing** ("where you are"), much like a home address that changes if you move. Used to identify and reach networks and hosts.
    - **IPv4:** 32 bits, divided into 4 octets separated by dots (e.g., 69.58.201.25).
    - **IPv6:** 128 bits, divided into 8 hexatets of 4 hexadecimal digits separated by colons.
