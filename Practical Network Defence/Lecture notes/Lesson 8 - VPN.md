...
- **Incoming Traffic:** If the packet contains an IPsec header perform a lookup in the SPD and retrieve its policy to check if it is supposed to be processed. If so the kernel extracts the SPI from the incoming IPsec header and looks up the corresponding SA in the SAD to decrypt and authenticate the packet. Before handing the data up to the transport layer, the kernel strictly checks the SPD to ensure the packet's protection perfectly matched the required security policy (dropping it if it fails).
