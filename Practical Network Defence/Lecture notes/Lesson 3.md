...
**2. IPv6 Address Types** IPv6 eliminates broadcast addresses entirely, replacing them with Multicast.
...
**4. ICMPv6 & Auto-Configuration** IPv6 relies heavily on **ICMPv6** to manage the network, completely replacing IPv4's ARP with the **Neighbor Discovery Protocol (NDP)**.
...
**5. DHCPv6 & Prefix Delegation** Router Advertisements contain flags (`M` for Managed, `O` for Other Configuration) that tell the host how to get its IP.
...
**6. Lab Activity Check-list**

- **Tools:** You will use **Kathará** (a container framework) to configure a Dual Stack (IPv4 and IPv6) topology.
- **Manual Configuration:** Use `ip` commands or the `/etc/network/interfaces` file.
- **Automated Services:** You will configure `radvd` (Routing Advertisement Daemon) to send RAs for SLAAC, and `dnsmasq` to act as a lightweight stateless DHCPv6 / DNS server.