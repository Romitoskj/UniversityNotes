...
- **The Identifier Space:** Identifiers (fingerprints) are treated as numbers in a circular space or **ring**, typically ranging from $0$ to $2^{256} - 1$ if using SHA-256.
- **Projection:** Both servers (using their IP addresses) and files (using their content hashes) are projected onto this ring.
